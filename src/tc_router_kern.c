...
static __always_inline int handle_ipv4(struct __sk_buff *skb, struct ethhdr *eth,
                                       void *data_end, bool dec_ttl)
{
...
int tc_router_ingress(struct __sk_buff *skb)
{
    ...
    if (eth->h_proto == __constant_htons(ETH_P_IP))
        return handle_ipv4(skb, eth, data_end, true);

    return BPF_OK;
}

SEC("classifier")
int tc_router_egress(struct __sk_buff *skb)
{
    void *data = (void *)(long)skb->data;
    void *data_end = (void *)(long)skb->data_end;
    ...
    if (eth->h_proto == __constant_htons(ETH_P_IP))
        return handle_ipv4(skb, eth, data_end, false);
